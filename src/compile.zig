...
pub fn compile(pattern: *const Pattern, opts: Options) !Database {
    ...
    return if (db) |ptr| Database.init(@ptrCast(ptr)) else error.UnknownError;
}
...
test Mode {
    // Test default mode
    try std.testing.expectEqual(0, try Mode.value(Mode{}));

    // Test individual modes
    try std.testing.expectEqual(hs.HS_MODE_BLOCK, try Mode.value(Mode{ .block = true }));
    try std.testing.expectEqual(hs.HS_MODE_STREAM, try Mode.value(Mode{ .stream = true }));
    try std.testing.expectEqual(hs.HS_MODE_VECTORED, try Mode.value(Mode{ .vectored = true }));

    // Test combinations of modes with SOM horizon flags
    try std.testing.expectEqual(hs.HS_MODE_STREAM | hs.HS_MODE_SOM_HORIZON_LARGE, try Mode.value(Mode{ .stream = true, .som_horizon_large = true }));
    try std.testing.expectEqual(hs.HS_MODE_STREAM | hs.HS_MODE_SOM_HORIZON_MEDIUM, try Mode.value(Mode{ .stream = true, .som_horizon_medium = true }));
    try std.testing.expectEqual(hs.HS_MODE_STREAM | hs.HS_MODE_SOM_HORIZON_SMALL, try Mode.value(Mode{ .stream = true, .som_horizon_small = true }));

    try std.testing.expectError(error.DbModeError, Mode.value(Mode{ .block = true, .stream = true }));
    try std.testing.expectError(error.DbModeError, Mode.value(Mode{ .block = true, .vectored = true }));
    try std.testing.expectError(error.DbModeError, Mode.value(Mode{ .stream = true, .vectored = true }));
    try std.testing.expectError(error.DbModeError, Mode.value(Mode{ .block = true, .stream = true, .vectored = true }));

    try std.testing.expectError(error.DbModeError, Mode.value(Mode{ .block = true, .som_horizon_large = true }));
    try std.testing.expectError(error.DbModeError, Mode.value(Mode{ .vectored = true, .som_horizon_medium = true }));
    try std.testing.expectError(error.DbModeError, Mode.value(Mode{ .block = true, .som_horizon_small = true }));

    try std.testing.expectError(error.DbModeError, Mode.value(Mode{ .stream = true, .som_horizon_large = true, .som_horizon_medium = true }));
    try std.testing.expectError(error.DbModeError, Mode.value(Mode{ .stream = true, .som_horizon_large = true, .som_horizon_small = true }));
    try std.testing.expectError(error.DbModeError, Mode.value(Mode{ .stream = true, .som_horizon_medium = true, .som_horizon_small = true }));
    try std.testing.expectError(error.DbModeError, Mode.value(Mode{ .stream = true, .som_horizon_large = true, .som_horizon_medium = true, .som_horizon_small = true }));
}
...
test "Mode round trip" {
    ...
    const test_modes = [_]Mode{
        ...
        Mode{ .stream = true },
        Mode{ .vectored = true },
        Mode{ .stream = true, .som_horizon_large = true },
        Mode{ .stream = true, .som_horizon_medium = true },
        Mode{ .stream = true, .som_horizon_small = true },
    };
...
test "compile error handling" {
    ...
    const malformed_pattern = try Pattern.parse("[");
    try std.testing.expectError(error.CompileError, compile(&malformed_pattern, .{}));

    // Test unbalanced parentheses
    const unbalanced_pattern = try Pattern.parse("(test");
...
test "compile_multi with different modes" {
    const patterns = [_]Pattern{
        try Pattern.parse("test1"),
        try Pattern.parse("test2"),
        try Pattern.parse("test3"),
    };

    // Test block mode
    const block_db = try compile_multi(&patterns, .{ .mode = .{ .block = true } });
    defer block_db.deinit();

    // Test stream mode
    const stream_db = try compile_multi(&patterns, .{ .mode = .{ .stream = true } });
    defer stream_db.deinit();

    // Test vectored mode
    const vectored_db = try compile_multi(&patterns, .{ .mode = .{ .vectored = true } });
    defer vectored_db.deinit();
}
...
test "compile_multi with different platforms" {
    ...
    // Test with generic platform
    const generic_db = try compile_multi(&patterns, .{ .platform = Platform{ .tune = .generic } });
    defer generic_db.deinit();

    // Test with specific platform
    const haswell_db = try compile_multi(&patterns, .{ .platform = Platform{ .tune = .haswell, .cpu_features = .avx2 } });
    defer haswell_db.deinit();
}

test "compile_multi with patterns with IDs" {
    const patterns = [_]Pattern{
        Pattern{ .expr = "test1", .id = 1 },
        Pattern{ .expr = "test2", .id = 2 },
        Pattern{ .expr = "test3", .id = 3 },
    };

    const db = try compile_multi(&patterns, .{});
    defer db.deinit();
}
...
test "compile_multi with mixed patterns" {
    const patterns = [_]Pattern{
        ...
        try Pattern.parse("/test3/i"),
        Pattern{ .expr = "test4", .id = 4, .flags = .{ .caseless = true } },
    };
...
test "compile_multi with empty pattern list" {
    const patterns = [_]Pattern{};
    try std.testing.expectError(error.CompileError, compile_multi(&patterns, .{}));
}

test "compile_multi with single pattern" {
    const patterns = [_]Pattern{try Pattern.parse("test")};
    const db = try compile_multi(&patterns, .{});
    defer db.deinit();
}
...
test "compile_multi with many patterns" {
    ...
    for (0..100) |i| {
        ...
        patterns[i] = try Pattern.parse(exprs[i]);
    }
    ...
    const db = try compile_multi(patterns[0..100], .{});
    defer db.deinit();
}
...
test "compile_multi error handling" {
    ...
    const invalid_patterns = [_]Pattern{
        try Pattern.parse("("),
        try Pattern.parse("test"),
    };
    try std.testing.expectError(error.CompileError, compile_multi(&invalid_patterns, .{}));
    ...
    try std.testing.expectError(error.CompileError, compile_multi(&malformed_patterns, .{}));
}

test "compile_multi with unicode patterns" {
    const patterns = [_]Pattern{
        try Pattern.parse("测试1"),
        try Pattern.parse("测试2"),
        try Pattern.parse("/测试3/8"),
    };
...
test "compile_multi with complex patterns" {
    const patterns = [_]Pattern{
        try Pattern.parse("\\b\\w+@\\w+\\.\\w+\\b"),
        try Pattern.parse("[a-z]+\\d*"),
        try Pattern.parse("^test$"),
        try Pattern.parse("a{3,5}"),
    };
...
test "compile_multi with different allocators" {
    ...
    // Test with testing allocator
    const test_db = try compile_multi(&patterns, .{ .allocator = std.testing.allocator });
    defer test_db.deinit();

    // Test with c allocator
    const c_db = try compile_multi(&patterns, .{ .allocator = std.heap.c_allocator });
    defer c_db.deinit();
}

test free_compile_error {
    // Test that free_compile_error handles null pointer correctly
    free_compile_error(null);

    // Test that free_compile_error is called during compile error
    const invalid_pattern = try Pattern.parse("f(bar");
    try std.testing.expectError(error.CompileError, compile(&invalid_pattern, .{}));
...
test "free_compile_error with multi-pattern errors" {
    // Test that free_compile_error is called during multi-pattern compile errors
    const invalid_patterns = [_]Pattern{
        try Pattern.parse("("),
        try Pattern.parse("test"),
    };
...
test "free_compile_error error message validation" {
...
test "integration - complex workflow" {
    ...
    const patterns = [_]Pattern{
        (try Pattern.parse("email")).withExt(.{ .min_offset = 10 }),
        try Pattern.parse("/phone/i"),
        Pattern{ .expr = "credit_card", .id = 1, .flags = .{ .caseless = true } },
        try Pattern.parse("\\b\\w+@\\w+\\.\\w+\\b"),
    };
...
test "integration - literal vs regex compilation" {
    const p = try Pattern.parse("test123");
...
test "integration - pattern with all features" {
    ...
    const db = try compile(&complex_pattern, .{
        .mode = .{ .block = true },
        .platform = Platform{ .tune = .skylake, .cpu_features = .avx512 },
        .literal = false,
    });
    defer db.deinit();
}
...
test "error handling - invalid patterns" {
    ...
    for (invalid_patterns) |pattern_str| {
        const pattern = try Pattern.parse(pattern_str);
...
test "error handling - memory allocation errors" {
    ...
    const res = compile_multi(&[_]Pattern{pattern}, .{ .allocator = fba.allocator() });
