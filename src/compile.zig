...
test "compile_multi with different platforms" {
    ...
    // Test with generic platform
    const generic_db = try compileMulti(&patterns, .{ .platform = Platform{ .tune = .generic } });
    defer generic_db.deinit();

    // Test with specific platform
    const haswell_db = try compileMulti(&patterns, .{ .platform = Platform{ .tune = .haswell, .cpu_features = .avx2 } });
    defer haswell_db.deinit();
}
